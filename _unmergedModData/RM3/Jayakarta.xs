...
void main(void)
{
   ...
   {
		subCiv0=rmGetCivID("Betawi");
		rmEchoInfo("subCiv0 is Betawi "+subCiv0);
      if (subCiv0 >= 0)
			rmSetSubCiv(0, "Betawi");
		
		subCiv1=rmGetCivID("PapiaTugu");
		rmEchoInfo("subCiv1 is PapiaTugu "+subCiv1);
		if (subCiv1 >= 0)
			rmSetSubCiv(1, "PapiaTugu");

		subCiv2=rmGetCivID("PapiaTugu");
		rmEchoInfo("subCiv2 is PapiaTugu "+subCiv2);
		if (subCiv2 >= 0)
			rmSetSubCiv(2, "PapiaTugu");

		subCiv3=rmGetCivID("Betawi");
		rmEchoInfo("subCiv3 is Betawi "+subCiv3);
		if (subCiv3 >= 0)
			rmSetSubCiv(3, "Betawi");
		
		if (rmRandFloat(0,1) < 0.5)
		{
			subCiv4=rmGetCivID("Betawi");
			rmEchoInfo("subCiv4 is Betawi "+subCiv4);
			if (subCiv4 >= 0)
				rmSetSubCiv(4, "Betawi");
		}
	}			
   ...
   rmSetSeaLevel(1.0);
	...
	float Lightset = rmRandFloat(0.0, 1.0);
  ...
  // Text
   rmSetStatusText("",0.40);
	...
	{  
		...
		int SeminolesVillageType = rmRandInt(1,3);
		SeminolesVillageID = rmCreateGrouping("Seminole village", "Betawi_"+SeminolesVillageType);
	...
	if (subCiv1 == rmGetCivID("PapiaTugu"))
   {   
	   int smallIslandNative2=rmCreateArea("small island native 2");
		...
		CherokeeVillageID = rmCreateGrouping("Cherokee village", "Papia_Tugu_"+CherokeeVillageType);
	...
	if (subCiv2 == rmGetCivID("PapiaTugu"))
   {   
		...
		int Cherokee2VillageType = rmRandInt(1,3);
		Cherokee2VillageID = rmCreateGrouping("Cherokee2 village", "Papia_Tugu_"+Cherokee2VillageType);
   ...
   {   
	   if ( cNumberTeams <= 2 )
	   {
		...
		int Seminoles2VillageType = rmRandInt(1,3);
		Seminoles2VillageID = rmCreateGrouping("Seminole2 village", "Betawi_"+Seminoles2VillageType);
...
*/
	...
	rmAddObjectDefItem(StartAreaTreeID, "STreeSundaKelapa", rmRandInt(5,8), 4.0);
	...
	rmAddObjectDefItem(startBerryID, "SMundu", 4, 5.0);
	...
	rmAddObjectDefConstraint(startBerryID, avoidResource);
		...
		rmAddObjectDefConstraint(SoldatSekulID, avoidStartingUnits);
...
// Text
   rmSetStatusText("",0.80);
	...
	rmAddObjectDefItem(deerID, "deer", rmRandInt(3,4), 5.0);
	...
	rmAddObjectDefItem(turkeyID, "Deer", rmRandInt(3,5), 5.0);
   ...
   for (i=0; <numTries)
      {   
         ...
         rmSetAreaSize(forest, rmAreaTilesToFraction(400), rmAreaTilesToFraction(600));
         rmSetAreaForestType(forest, "Jakarta Forest");
    ...
    // Embellishments
	int avoidEagles=rmCreateTypeDistanceConstraint("avoids Eagles", "Alligator", 40.0);

	int randomEagleTreeID=rmCreateObjectDef("random eagle tree");
	rmAddObjectDefItem(randomEagleTreeID, "Alligator", 1, 0.0);
	...
	rmAddObjectDefItem(randomWaterTreeID, "STreeMeranti", 3, 0.0);
	...
	rmAddObjectDefItem(fishID, "SMosquito", 1, 0.0);
	rmSetObjectDefMinDistance(fishID, 0.0);
	rmSetObjectDefMaxDistance(fishID, rmXFractionToMeters(0.5));
	rmAddObjectDefConstraint(fishID, fishVsFishID);
	rmAddObjectDefConstraint(fishID, treeVsLand);
