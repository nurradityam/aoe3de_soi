...
void main(void)
{
  ...
  int avoidTCMedium=rmCreateTypeDistanceConstraint("stay away from TC by a bit", "TownCenter", 8.0);
   ...
   int smallislandConstraint=rmCreateClassDistanceConstraint("islands avoid each other", classIsland, 10.0);
  // int bonusIslandConstraint=rmCreateClassDistanceConstraint("bonus islands avoid islands", classIsland, 30.0);
   ...
   int smallavoidTeamIslands=rmCreateClassDistanceConstraint("stuff avoids team islands", classTeamIsland, 21.0);
   int landflag = rmCreateTypeDistanceConstraint("land against flag", "HomeCityWaterSpawnFlag", 7.0);
   ...
   int avoidGold=rmCreateTypeDistanceConstraint("avoid coin", "Gold", 25.0);
   int avoidNugget=rmCreateTypeDistanceConstraint("nugget avoid nugget", "abstractNugget", 50.0);
   int avoidNuggetshort=rmCreateTypeDistanceConstraint("nugget avoid nugget", "abstractNugget", 20.0);
   ...
   //int flagEdgeConstraint = rmCreatePieConstraint("flags away from edge of map", 0.5, 0.5, rmGetMapXSize()-200, rmGetMapXSize()-100, 0, 0, 0);
		int flagEdgeConstraint = rmCreatePieConstraint("flags away from edge of map", 0.5, 0.5, rmGetMapXSize()-100, rmGetMapXSize()-100, 0, 0, 0);
...
//	else
  ...
  float yLoc = 0.0;
	...
	float isleSize = (0.15 / cNumberTeams);
   ...
   for(i=0; <cNumberTeams)
   {
      ...
      rmSetAreaMinBlobs(teamID, 4);
      rmSetAreaMaxBlobs(teamID, 8);
      rmSetAreaMinBlobDistance(teamID, 15.0);
      rmSetAreaMaxBlobDistance(teamID, 20.0);
		rmSetAreaCoherence(teamID, 0.35);
      rmSetAreaBaseHeight(teamID, 2.0);
      rmSetAreaSmoothDistance(teamID, 0);
		rmSetAreaMix(teamID, "borneo_grass_a");
...
//	rmAddAreaConstraint(teamID, playerEdgeConstraint);
   ...
   {  
	  ...
	  if (cNumberTeams > 2)
		caribsVillageType = rmRandInt(1,4);
      caribsVillageID = rmCreateGrouping("caribs city", "Matraman_"+caribsVillageType);
	...
	if (cNumberTeams == 2)
	{
		...
		{  
			...
			int caribs2VillageType = rmRandInt(1,4);
			if (caribsVillageType == 3)
				caribs2VillageType = rmRandInt(1,2);
			caribs2VillageID = rmCreateGrouping("caribs2 city", "Matraman_"+caribs2VillageType);
		...
		{  
			...
			int caribs3VillageType = rmRandInt(1,4);
			caribs3VillageID = rmCreateGrouping("caribs3 city", "Matraman_"+caribs3VillageType);
  ...
  rmAddObjectDefItem(playerDeerID, "Pronghorn", rmRandInt(10,15), 10.0);
  ...
  //      rmAddObjectDefItem(colonyShipID, "caravel", 1, 10.0);
   // }
    ...
    rmPlaceObjectDefAtLoc(colonyShipID, i, 0.5, 0.5);
   ...
   rmAddObjectDefConstraint(silverID, avoidImpassableLand);
	...
	float isletSize = rmAreaTilesToFraction(200);
	for(i=1; <40)
   {
	int nat1 = rmCreateArea("east island"+i);
      rmSetAreaSize(nat1, isletSize, isletSize);
      rmSetAreaMix(nat1, "borneo_grass_a");
      rmAddAreaToClass(nat1, classNeutralIsland);
      rmAddAreaToClass(nat1, classIsland);
      rmSetAreaCoherence(nat1, 0.5);
      rmSetAreaSmoothDistance(nat1, 3);
	  rmSetAreaBaseHeight(nat1, 3.0);
      rmAddAreaConstraint(nat1, smallislandConstraint);
      rmAddAreaConstraint(nat1, smallavoidTeamIslands);
	  rmAddAreaConstraint(nat1, playerEdgeConstraint);
      rmAddAreaConstraint(nat1, landflag);
      rmSetAreaMinBlobs(nat1, 2);
      rmSetAreaMaxBlobs(nat1, 5);
			rmSetAreaMinBlobDistance(nat1, 4.0);
			rmSetAreaMaxBlobDistance(nat1, 9.0);
	  rmSetAreaElevationType(nat1, cElevTurbulence);
      rmSetAreaElevationVariation(nat1, 5.0);
      rmSetAreaElevationMinFrequency(nat1, 0.10);
      rmSetAreaElevationOctaves(nat1, 3);
      rmSetAreaElevationPersistence(nat1, 0.2);
      rmSetAreaElevationNoiseBias(nat1, 1);
   ...
   int mineneutralID=rmCreateObjectDef("neutral mine");
   rmAddObjectDefItem(mineneutralID, "SKapuk", 6, 3.0);
   rmSetObjectDefMinDistance(mineneutralID, 1);
   rmSetObjectDefMaxDistance(mineneutralID, 1);
   rmAddObjectDefConstraint(mineneutralID, avoidGold);
   rmAddObjectDefConstraint(mineneutralID, avoidTeamIslands);
   rmAddObjectDefConstraint(mineneutralID, avoidBonusIslands);
   rmAddObjectDefConstraint(mineneutralID, shortAvoidImpassableLand);
   	for(i=1; <3) 
      {rmPlaceObjectDefInRandomAreaOfClass(mineneutralID, 0, classNeutralIsland);}
	  
   // Define and place trees to random islands   
   int treesID=rmCreateObjectDef("trees");
   rmAddObjectDefItem(treesID, "STreeMeranti", rmRandInt(6,7), 4.0);
   rmSetObjectDefMinDistance(treesID, 2);
   rmSetObjectDefMaxDistance(treesID, 4);
   rmAddObjectDefConstraint(treesID, shortAvoidImpassableLand);
   	for(i=1; <70) 
      {rmPlaceObjectDefInRandomAreaOfClass(treesID, 0, classNeutralIsland);}
	  
	  
	int nuggetbonus= rmCreateObjectDef("nugget bonus"); 
	rmAddObjectDefItem(nuggetbonus, "Nugget", 1, 0.0);
	rmSetObjectDefMinDistance(nuggetbonus, 0.0);
	rmSetNuggetDifficulty(4, 4);
	rmSetObjectDefMaxDistance(nuggetbonus, rmXFractionToMeters(0.5));
	rmAddObjectDefConstraint(nuggetbonus, shortAvoidImpassableLand);
  	rmAddObjectDefConstraint(nuggetbonus, avoidNuggetshort);
  	rmAddObjectDefConstraint(nuggetbonus, avoidTradeRoute);
  	rmAddObjectDefConstraint(nuggetbonus, avoidAll);
	rmAddObjectDefConstraint(nuggetbonus, avoidTCshort);
  	rmAddObjectDefConstraint(nuggetbonus, avoidWater20);
	rmAddObjectDefConstraint(nuggetbonus, playerEdgeConstraint);
	rmAddObjectDefConstraint(nuggetbonus, avoidTeamIslands);
	//rmPlaceObjectDefInArea(nuggetbonus, 0, bigIslandID, cNumberNonGaiaPlayers*rmRandInt(1,2));
	for (i=1; <5)
	{
		rmPlaceObjectDefInRandomAreaOfClass(nuggetbonus, 0, classBonusIsland);
	}
 ...
 // Define and place Nuggets
  ...
  if(rmGetIsKOTH()) {
   ...
   if(bonusChance<0.5)   
      rmAddObjectDefItem(deerID, "Pronghorn", rmRandInt(4,6), 10.0);
   else
      rmAddObjectDefItem(deerID, "Pronghorn", rmRandInt(8,10), 10.0);
...
//	rmPlaceObjectDefAtLoc(deerID, 0, 0.5, 0.5, cNumberNonGaiaPlayers*4);
   ...
   rmAddObjectDefItem(randomTreeID, "STreeMeranti", 1, 0.0);
  ...
  int avoidNuggetWater=rmCreateTypeDistanceConstraint("nugget vs. nugget water", "AbstractNugget", 35.0);
