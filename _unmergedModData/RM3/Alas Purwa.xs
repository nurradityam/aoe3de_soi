...
void main(void)
{
   ...
   if (cNumberNonGaiaPlayers >6)
	...
	rmTerrainInitialize("amazon grass", 5);
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------  
   ...
   rmSetObjectDefMaxDistance(socketID, 8.0);
		...
		rmAddRandomTradeRouteWaypoints(tradeRouteID, 0.5, 0.5);
		rmAddRandomTradeRouteWaypoints(tradeRouteID, 0.7, 0.4);
		rmAddRandomTradeRouteWaypoints(tradeRouteID, 1.0, 0.25);
   ...
   rmAddObjectDefToClass(socket2ID, rmClassID("socketClass"));
   ...
   rmSetObjectDefMaxDistance(socket2ID, 8.0);
		...
		rmAddRandomTradeRouteWaypoints(tradeRoute2ID, 0.5, 0.5);
		rmAddRandomTradeRouteWaypoints(tradeRoute2ID, 0.3, 0.4);
		rmAddRandomTradeRouteWaypoints(tradeRoute2ID, 0.00, 0.25);
   ...
   if(placedTradeRoute2 == false)
	...
	// add the sockets along the trade route.
   vector socketLoc2 = rmGetTradeRouteWayPoint(tradeRoute2ID, 0.3);
   ...
   */
   ...
   rmSetAreaSmoothDistance(hillNorthwestID, 10);
	rmSetAreaMix(hillNorthwestID, "amazon grass");
	rmAddAreaTerrainLayer(hillNorthwestID, "amazon grass", 0, 4);
   rmAddAreaTerrainLayer(hillNorthwestID, "amazon grass", 4, 10);
   ...
   rmSetAreaSmoothDistance(hillSoutheastID, 10);
	rmSetAreaMix(hillSoutheastID, "amazon grass");
	rmAddAreaTerrainLayer(hillSoutheastID, "amazon grass", 0, 4);
   rmAddAreaTerrainLayer(hillSoutheastID, "amazon grass", 4, 10);
   ...
   // Natives -> Secrets -> Nuggets -> Small Ponds (whee)
	...
	rmAddObjectDefItem(StartBisonID, "Deer", 4, 4.0);
	...
	rmAddObjectDefItem(StartBerryID, "SPepaya", 4, 4.0);
	rmSetObjectDefMinDistance(StartBerryID, 6.0);
	rmSetObjectDefMaxDistance(StartBerryID, 8.0);
	...
	rmAddObjectDefConstraint(SoldatSekulID, avoidStartingUnitsSmall);
	...
	if (rmGetPlayerCiv(i) ==  rmGetCivID("Germans")) {
          rmPlaceObjectDefAtLoc(SoldatSekulID, i, rmXMetersToFraction(i), rmZMetersToFraction(i));
        }
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------  
   ...
   rmAddObjectDefItem(bisonID, "Deer", rmRandInt(3,4), 3.0);
   ...
   rmAddObjectDefItem(pronghornID, "Pronghorn", rmRandInt(2,3), 3.0);
   ...
   for(i=1; <30)
   {
		...
		rmSetAreaSize(treeArea, 0.005, 0.009);
		rmSetAreaForestType(treeArea, "Jawa Meranti Forest");
   ...
   for (i=0; <numTries)
   {   
		int forestr=rmCreateArea("random forest "+i);
		rmSetAreaWarnFailure(forestr, false);
		rmSetAreaSize(forestr, rmAreaTilesToFraction(200), rmAreaTilesToFraction(200));
		rmSetAreaForestType(forestr, "Jawa Lowland Forest");
		rmSetAreaForestDensity(forestr, 0.8);
		rmSetAreaForestClumpiness(forestr, 0.4);
		rmSetAreaForestUnderbrush(forestr, 0.9);
		rmSetAreaCoherence(forestr, 0.5);
		// rmSetAreaSmoothDistance(forest, 10);
		rmAddAreaToClass(forestr, rmClassID("classForest")); 
		// rmAddAreaConstraint(forest, lowForestConstraint);
		rmAddAreaConstraint(forestr, forestConstraint);
		rmAddAreaConstraint(forestr, avoidImportantItem);
		rmAddAreaConstraint(forestr, avoidImpassableLand); 
		rmAddAreaConstraint(forestr, avoidSocket);
		rmAddAreaConstraint(forestr, avoidNuggetSmall);
		// rmAddAreaConstraint(forest, avoidFastCoin);
		rmAddAreaConstraint(forestr, avoidStartingUnits);
		if(rmBuildArea(forestr)==false)
		{
			// Stop trying once we fail 10 times in a row.
			failCount++;
			if(failCount==10)
