...
void main(void)
{
	...
	// Which trade route?
	int whichTradeRoute = rmRandInt(2,3);  // On a 1, do the funky mini-route in the middle.
   ...
   {
      subCiv0=rmGetCivID("Wetanan");
      rmEchoInfo("subCiv0 is Wetanan "+subCiv0);
      if (subCiv0 >= 0)
         rmSetSubCiv(0, "Wetanan");

		subCiv1=rmGetCivID("Wetanan");
      rmEchoInfo("subCiv1 is Wetanan "+subCiv1);
		if (subCiv1 >= 0)
			 rmSetSubCiv(1, "Wetanan");
   ...
   rmSetSeaLevel(1.0);
	...
	rmTerrainInitialize("texas\ground1_tex",10);
	rmSetMapType("jancukland");
	rmSetMapType("water");
	rmSetMapType("grass");
	// rmSetGlobalSnow( 0.7 );
   ...
   int avoidElk=rmCreateTypeDistanceConstraint("elk avoids elk", "pronghorn", 35.0);
   int avoidMoose=rmCreateTypeDistanceConstraint("moose avoids moose", "SBangauTongtong", 30.0);
   int fishVsFishID=rmCreateTypeDistanceConstraint("fish v fish", "fish", 18.0);
   int whaleVsWhaleID=rmCreateTypeDistanceConstraint("whale v whale", "HumpbackWhale", 15.0);
   int fishLand = rmCreateTerrainDistanceConstraint("fish land", "land", true, 6.0);
   int whaleLand = rmCreateTerrainDistanceConstraint("whale v. land", "land", true, 15.0);
   int avoidSheep=rmCreateTypeDistanceConstraint("sheep avoids sheep", "ypGoat", 40.0);
   ...
   rmSetAreaElevationPersistence(OuterLandID, 0.4);
	rmSetAreaMix(OuterLandID, "borneo_grass_a");
   ...
   rmSetAreaMaxBlobDistance(InnerLandID, 20);
	rmSetAreaMix(InnerLandID, "borneo_grass_a");
   ...
   rmSetAreaWaterType(HudsonBayID, "Ceylon Coast");
   ...
   */
   ...
   // Define and place the river brantas
  int riverID = rmRiverCreate(-1, "Surabaya Water", 5, 8, 6, 7);
  rmRiverAddWaypoint(riverID, 0.0, 0.0);
	rmRiverAddWaypoint(riverID, 0.6, 0.6);
   ...
   float NativeVillageLoc = rmRandFloat(0,0.4);
     
	int CreeVillageAID = -1;
   int CreeVillageType = rmRandInt(1,5);
   CreeVillageAID = rmCreateGrouping("Cree village A", "Surabayan_"+CreeVillageType);
   ...
   rmAddGroupingConstraint(CreeVillageAID, avoidRiver);
   ...
   rmAddGroupingConstraint(CreeVillageAID, avoidImportantItem);
	...
	{
		if(NativeVillageLoc < 0.25)
			rmPlaceGroupingAtLoc(CreeVillageAID, 0, 0.3, 0.2);
		else if(NativeVillageLoc < 0.5)
			rmPlaceGroupingAtLoc(CreeVillageAID, 0, 0.2, 0.3); // DAL
		else if(NativeVillageLoc < 0.75)
			rmPlaceGroupingAtLoc(CreeVillageAID, 0, 0.5, 0.9); // DAL
		else
   ...
   CreeVillageBID = rmCreateGrouping("Cree village B", "Surabayan_"+CreeVillageType);
   ...
   rmAddGroupingConstraint(CreeVillageBID, avoidRiver);
   ...
   rmAddGroupingConstraint(CreeVillageBID, avoidImportantItem);
	...
	else
	{
		if(NativeVillageLoc < 0.25)
			...
			rmPlaceGroupingAtLoc(CreeVillageBID, 0, 0.3, 0.2);
		else if(NativeVillageLoc < 0.75)
   ...
   huronVillageAID = rmCreateGrouping("huron village A", "Kejawen_"+huronVillageType);
   ...
   rmAddGroupingConstraint(huronVillageAID, avoidRiver);
   ...
   rmAddGroupingConstraint(huronVillageAID, avoidImportantItem);
	...
	else
	{
		if(NativeVillageLoc < 0.25)
			...
			rmPlaceGroupingAtLoc(huronVillageAID, 0, 0.8, 0.5);
		else if(NativeVillageLoc < 0.75)
			rmPlaceGroupingAtLoc(huronVillageAID, 0, 0.9, 0.6); //DAL
		else
			rmPlaceGroupingAtLoc(huronVillageAID, 0, 0.3, 0.2);
	}

	int huronVillageBID = -1;
   huronVillageType = rmRandInt(1,5);
   huronVillageBID = rmCreateGrouping("huron village B", "Kejawen_"+huronVillageType);
   ...
   rmAddGroupingConstraint(huronVillageBID, avoidRiver);
   ...
   rmAddGroupingConstraint(huronVillageBID, avoidTradeRoute);
	...
	{
		if(NativeVillageLoc < 0.25)
			rmPlaceGroupingAtLoc(huronVillageBID, 0, 0.2, 0.3);
		else if(NativeVillageLoc < 0.5)
			rmPlaceGroupingAtLoc(huronVillageBID, 0, 0.6, 0.9);	//DAL
		else if(NativeVillageLoc < 0.75)
			rmPlaceGroupingAtLoc(huronVillageBID, 0, 0.3, 0.2);
		else
			rmPlaceGroupingAtLoc(huronVillageBID, 0, 0.9, 0.6); //DAL
	}
	...
	rmAddObjectDefConstraint(startingTCID, avoidRiver);

	int StartAreaTreeID=rmCreateObjectDef("starting trees");
	rmAddObjectDefItem(StartAreaTreeID, "STreeMahogany", 1, 0.0);
  ...
  int startingPanggung=rmCreateObjectDef("Panggung Krapyak");
  rmAddObjectDefItem(startingPanggung, "PanggungKrapyak", 1, 0.0);
  rmSetObjectDefMinDistance(startingPanggung, 20.0);
  rmSetObjectDefMaxDistance(startingPanggung, 22.0);
	rmAddObjectDefConstraint(startingPanggung, avoidStartingUnitsSmall);
	rmAddObjectDefToClass(startingPanggung, rmClassID("startingUnit"));
	
  int startingcoffee=rmCreateObjectDef("Coffee Plant");
  rmAddObjectDefItem(startingcoffee, "ypRicePaddy", 1, 0.0);
  rmSetObjectDefMinDistance(startingcoffee, 20.0);
  rmSetObjectDefMaxDistance(startingcoffee, 22.0);
	rmAddObjectDefConstraint(startingcoffee, avoidStartingUnitsSmall);
	rmAddObjectDefToClass(startingcoffee, rmClassID("startingUnit"));
  ...
  int silverID = -1;
	...
	for(i=1; <cNumberPlayers)
	{
        ...
        rmPlaceObjectDefAtLoc(ypMonasteryBuilder(i, 1), i, rmXMetersToFraction(xsVectorGetX(TCLocation)), rmZMetersToFraction(xsVectorGetZ(TCLocation)));
						...
						rmGetPlayerCiv(i) ==  rmGetCivID("Ottomans") ||
						rmGetPlayerCiv(i) ==  rmGetCivID("Pirate") ||
						rmGetPlayerCiv(i) ==  rmGetCivID("Spanish") ||
						rmGetPlayerCiv(i) ==  rmGetCivID("French") ||
						rmGetPlayerCiv(i) ==  rmGetCivID("Russians"))
			{
				rmPlaceObjectDefAtLoc(startingPanggung, i, rmXMetersToFraction(xsVectorGetX(TCLocation)), rmZMetersToFraction(xsVectorGetZ(TCLocation)));
			}
			else if (rmGetPlayerCiv(i) ==  rmGetCivID("British"))
			{
				rmPlaceObjectDefAtLoc(startingcoffee, i, rmXMetersToFraction(xsVectorGetX(TCLocation)), rmZMetersToFraction(xsVectorGetZ(TCLocation)));
			}
			else
			{
   ...
   int nuggetID= rmCreateObjectDef("nugget"); 
	...
	rmAddObjectDefConstraint(nuggetID, avoidRiver);
   ...
   rmAddObjectDefConstraint(whaleID, avoidRiver);
   ...
   // Place resources that we want forests to avoid
	...
	for(i=0; < silverCount)
	{
		...
		rmAddObjectDefConstraint(silverID, avoidRiver);
   ...
   for (i=0; <numTries)
      {   
         ...
         rmAddAreaConstraint(forest, avoidTradeRoute); 
			...
			rmAddAreaConstraint(forest, avoidRiver); 
   ...
   int elkID=rmCreateObjectDef("elk herd");
	rmAddObjectDefItem(elkID, "pronghorn", rmRandInt(3,4), 3.0);
   ...
   rmAddObjectDefConstraint(elkID, avoidImpassableLand);
	...
	rmAddObjectDefConstraint(elkID, avoidRiver);
   ...
   int mooseID=rmCreateObjectDef("moose herd");
	rmAddObjectDefItem(mooseID, "SBangauTongtong", rmRandInt(2,3), 4.0);
   ...
   rmAddObjectDefConstraint(mooseID, avoidImpassableLand);
	...
	rmAddObjectDefConstraint(mooseID, avoidRiver);
	rmAddObjectDefConstraint(mooseID, avoidStartingUnits);

   rmSetObjectDefCreateHerd(mooseID, true);
	rmPlaceObjectDefAtLoc(mooseID, 0, 0.5, 0.5, cNumberNonGaiaPlayers*8);
   ...
   rmAddObjectDefItem(sheepID, "ypGoat", 2, 3.0);
   ...
   rmAddObjectDefConstraint(sheepID, avoidRiver);
