...
extern int  gFortressUnit = cUnitTypeFortFrontier;    // The Asians have a different type.
...
void initArrays(void)
{
   ...
   gMapNames = xsArrayCreateString(50, "", "Map names");
     ...
     if (aiGetGameMode() == cGameModeDeathmatch) {
      gConsulateTechsSize = 56;
     }else{
      gConsulateTechsSize = 57;
     }
     //...BHG: JFR:
     ...
     gConsulateTechs = xsArrayCreateInt(40, 0, "Consulate Tech IDs");
        ...
        xsArraySetInt(gConsulateTechs, 31, cTechypConsulateSpanishMercantilism);  //this is the last original tech
        xsArraySetInt(gConsulateTechs, 32, cTechdhaengjajar);
        xsArraySetInt(gConsulateTechs, 33, cTechdhaengbekel);
        xsArraySetInt(gConsulateTechs, 34, cTechdhaengpanji);
        xsArraySetInt(gConsulateTechs, 35, cTechketanggung);
        xsArraySetInt(gConsulateTechs, 36, cTechketanggungagung);
        xsArraySetInt(gConsulateTechs, 37, cTechmantrijajar);
        xsArraySetInt(gConsulateTechs, 38, cTechmantribekel);
        xsArraySetInt(gConsulateTechs, 39, cTechlangenastra);
        xsArraySetInt(gConsulateTechs, 40, cTechlangenastraagung);
        xsArraySetInt(gConsulateTechs, 41, cTechplangkir);
        xsArraySetInt(gConsulateTechs, 42, cTechplangkirpakualaman);
        xsArraySetInt(gConsulateTechs, 43, cTechnyutrajajar);
        xsArraySetInt(gConsulateTechs, 44, cTechnyutrabekel);
        xsArraySetInt(gConsulateTechs, 45, cTechnyutrapanji);
        xsArraySetInt(gConsulateTechs, 46, cTechwirabrajajajar);
        xsArraySetInt(gConsulateTechs, 47, cTechwirabrajabekel);
        xsArraySetInt(gConsulateTechs, 48, cTechwirabrajapanji);
        xsArraySetInt(gConsulateTechs, 49, cTechlombokabang);
        xsArraySetInt(gConsulateTechs, 50, cTechlombokabangageng);
        xsArraySetInt(gConsulateTechs, 51, cTechjagerbekel);
        xsArraySetInt(gConsulateTechs, 52, cTechjagerpanji);
        xsArraySetInt(gConsulateTechs, 53, cTechsumoatmajabekel);
        xsArraySetInt(gConsulateTechs, 54, cTechsumoatmajapanji);
        xsArraySetInt(gConsulateTechs, 55, cTechsvantofel);
        xsArraySetInt(gConsulateTechs, 56, cTechswesikuning);
        //BHG: JFR: if we are in deatmatch we dont want to research the consulate tech that takes us to age 5...
        if (aiGetGameMode() != cGameModeDeathmatch) {        
         xsArraySetInt(gConsulateTechs, 57, cTechypConsulateJapaneseMeijiRestoration);
        }
        //...BHG: JFR:
...
void chooseConsulateFlag()
{
  ...
  if( (kbGetCiv() == cCivJapanese) || (kbGetCiv() == cCivSPCJapanese) || (kbGetCiv() == cCivSPCJapanese) ) {
    ...
    if (sponsor < 2) {
      flag_button_id = cTechsfactionpakualaman;
    }
    else {
      flag_button_id = cTechsfactionpepatihdalem;
    }
  }
...
void setConsulateArmyPreference() {
  ...
  if (kbTechGetStatus(cTechsfactionpakualaman) == cTechStatusActive) {
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSSekatenArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSMaulidNabiArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSPangeranGuardArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSKotagedeArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSRatuServantArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSPakualamanArmy, 0.6);
  }
  if (kbTechGetStatus(cTechsfactionpepatihdalem) == cTechStatusActive) {
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSSekatenArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSMaulidNabiArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSPangeranGuardArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSMaduraArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSRatuServantArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSHeirloomArmy, 0.6);
  }
...
int createSimpleBuildPlan(int puid=-1, int number=1, int pri=100, bool economy=true, int escrowID=-1, int baseID=-1, int numberBuilders=1)
{
   ...
   for (i=0; < number)
   {
	  ...
	  if (puid == cUnitTypeSoldatSekul)
      {
        //if (kbUnitCount(cMyID, cUnitTypeSoldatSekulWagon, cUnitStateAlive) > 0)
          //builderType = cUnitTypeSoldatSekulWagon;
		  aiPlanAddUnitType(planID, cUnitTypeSoldatSekulWagon, 1, 1, 1);
      }
...
void updateEscrows(void)
{
   ...
   {
      kbEscrowSetCap(cEconomyEscrowID, cResourceGold, 150); // Needed for factories
      kbEscrowSetCap(cEconomyEscrowID, cResourceWood, 350);
   }
...
vector selectForwardBaseLocation(void)
{
   ...
   if (dist > 0.0)
   {
      for (step = 0; < 9)
      {
         ...
         if ( getUnitByLocation(gFortressUnit, cPlayerRelationEnemy, cUnitStateABQ, retVal, 60.0) >= 0 )
   ...
   switch(gForwardBaseState)
   {
      case cForwardBaseStateNone:
      {
         ...
         {  // Yes.
            ...
            aiPlanSetVariableInt(gForwardBaseBuildPlan, cBuildPlanBuildingTypeID, 0, gFortressUnit);
            ...
            // Add position influence for nearby towers
            aiPlanSetVariableInt(gForwardBaseBuildPlan, cBuildPlanInfluenceUnitTypeID, 0, gFortressUnit);   // Don't build anywhere near another fort.
      ...
      {
         int fortUnitID = getUnitByLocation(gFortressUnit, cMyID, cUnitStateAlive, gForwardBaseLocation, 100.0);
         if ( fortUnitID >= 0 )
      ...
      {  // Normal state.  If fort is destroyed and base overrun, bail.
         if ( getUnitByLocation(gFortressUnit, cMyID, cUnitStateAlive, gForwardBaseLocation, 50.0) < 0 )
         {
...
mininterval 15
{
   ...
   if (givenFishingBoats == false)  // Skip these early-outs if we were granted free boats.
   {
      ...
      if ( (kbGetCiv() == cCivGermans) && (kbUnitCount(cMyID, cUnitTypeSugarFactory, cUnitStateABQ) < 1) )
         return;  // Don't burn wood before we have a sugar factory
      if ( (kbGetCiv() == cCivRussians) && (kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ) < 1) )
         return;  // Don't burn wood before we have a sawah
...
void initEcon(void)
{
   ...
   if ( (kbGetCiv() == cCivGermans) || (kbGetCiv() == cCivOttomans) )
      gHouseUnit = cUnitTypeHouseAztec;
...
//      effectiveAge = cAge2;
   ...
   switch(kbGetAge())
   {
      case cAge1:
      {
         ...
         {
            if (kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ) < 1)
               addItemToForecasts(cUnitTypeSSawah, 1 - kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ));
         }
      ...
      case cAge2:
      {
         ...
         if (cMyCiv == cCivRussians)
         {
            if (kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ) < 4)
               addItemToForecasts(cUnitTypeSSawah, 4 - kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ));
         }
      ...
      case cAge3:
      {
		 ...
		 // Germans - 3 sugar factory
         if (cMyCiv == cCivGermans)
         {
            if (kbUnitCount(cMyID, cUnitTypeSugarFactory, cUnitStateABQ) < 3)
               addItemToForecasts(cUnitTypeSugarFactory, 3 - kbUnitCount(cMyID, cUnitTypeSugarFactory, cUnitStateABQ));
         }
		 
		 // Russians - 8 sugar factory
         if (cMyCiv == cCivRussians)
         {
            if (kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ) < 8)
               addItemToForecasts(cUnitTypeSSawah, 8 - kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ));
         }
      ...
      case cAge4:
      {
		 ...
		 // Germans - 4 sugar factory
         if (cMyCiv == cCivGermans)
         {
            if (kbUnitCount(cMyID, cUnitTypeSugarFactory, cUnitStateABQ) < 4)
               addItemToForecasts(cUnitTypeSugarFactory, 4 - kbUnitCount(cMyID, cUnitTypeSugarFactory, cUnitStateABQ));
         }
		 
		 // Germans - 16 sawah
         if (cMyCiv == cCivRussians)
         {
            if (kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ) < 16)
               addItemToForecasts(cUnitTypeSSawah, 16 - kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ));
         }
      ...
      case cAge5:
      {
         ...
         if (cMyCiv == cCivRussians)
         {
            if (kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ) < 20)
               addItemToForecasts(cUnitTypeSSawah, 20 - kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ));
         }
...
{ 
  ...
  if (kbTechGetStatus(cTechsfactionpakualaman) == cTechStatusActive) {
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSSekatenArmy, 0.8);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSMaulidNabiArmy, 0.4);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSPangeranGuardArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSKotagedeArmy, 0.4);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSRatuServantArmy, 0.4);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSPakualamanArmy, 0.6);
  }
  if (kbTechGetStatus(cTechsfactionpepatihdalem) == cTechStatusActive) {
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSSekatenArmy, 0.4);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSMaulidNabiArmy, 0.8);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSPangeranGuardArmy, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSMaduraArmy, 0.4);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSRatuServantArmy, 0.4);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSHeirloomArmy, 0.8);
  }
  }
  
   if  (kbGetCiv() == cCivDutch)  {
	
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeMusketeer, 0.7);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSZwarte, 0.6);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSFlankeur, 0.8);
  }
  
   if  (kbGetCiv() == cCivOttomans)  {
	
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSTamtama, 0.8);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSPrawira, 0.8);
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeSMargatama, 0.8);
  }
...
//   gSettlerMaintainPlan = createSimpleMaintainPlan(gEconUnit, 25 - (13.0*btRushBoom), true, kbBaseGetMainID(cMyID), 1);  // Good through age 2
  ...
  if ( kbGetCiv() == cCivGermans ) {
    if (kbUnitCount(cMyID, cUnitTypeSoldatSekul, cUnitStateAlive) < 1) {
      createSimpleBuildPlan(cUnitTypeSoldatSekul, 1, 100, true, cMilitaryEscrowID, kbBaseGetMainID(cMyID), 1);
    }
  }
...
//rule useSoldatSekulWagon
//inactive
//minInterval 20
//{
  // int wagon = getUnit(cUnitTypeSoldatSekulWagon, cMyID, cUnitStateAlive);
   
   // Check if we have a factory wagon, but no factory build plan....
 //  if ( (wagon >= 0) && (aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeSoldatSekul) < 0) )
 //  {
 //     int planID = createSimpleBuildPlan(cUnitTypeSoldatSekul, 1, 50, true, cEconomyEscrowID, kbBaseGetMainID(cMyID), 0); //No settler builders
 //     aiPlanAddUnitType(planID, cUnitTypeSoldatSekulWagon, 1, 1, 1);
 //     xsEnableRule("getSteamPower");
//   }
//}
...
{ 
   ...
   if ( (kbGetCiv() == cCivGermans) && (kbUnitCount(cMyID, cUnitTypeSugarFactory, cUnitStateABQ) < 1) && (kbUnitCount(cMyID, gHouseUnit, cUnitStateABQ) >= 4))
      return;  // Dutch need bank before getting extra houses
...
minInterval 3
{
   ...
   if (kbGetCiv() == cCivRussians)
   {
      planID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeSSawah);
      if ( (planID < 0) && (kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateAlive) < 4) )  // If I'm not building one and I could be, do it.
      {     // Start a new one
         if ( kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateAlive) < ((kbGetAge()*2) + 1) )
         {  // Less than 1 bank in age 1 (0*2+1), less than 3 in age 2, etc.
            createSimpleBuildPlan(cUnitTypeSSawah, 1, 93, true, cEconomyEscrowID, kbBaseGetMainID(cMyID), 1); // Very high pri, just above houses
            aiEcho("Starting a new bank build plan.");
         }
      }     
      planID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, gMarketUnit);   // Dutch market check...only if bank exists
      if ( (planID < 0) && (kbUnitCount(cMyID, gMarketUnit, cUnitStateAlive) < 1) && (kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateAlive) > 0) )
      {
         createSimpleBuildPlan(gMarketUnit, 1, 96, true, cEconomyEscrowID, kbBaseGetMainID(cMyID), 1);  // Just higher than house
         aiEcho("Starting a new market build plan.");
      }
   ...
   if ( (kbGetCiv() == cCivRussians) && (kbUnitCount(cMyID, cUnitTypeSSawah, cUnitStateABQ) < 1) )
      return;
		...
		//edit SOI 9-12-2015/10:23
	 // if Germans, add sugar factory to the build limit.
   if (kbGetCiv() == cCivGermans)
   {
      planID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeSugarFactory);
      if ( (planID < 0) && (kbUnitCount(cMyID, cUnitTypeSugarFactory, cUnitStateAlive) < kbGetBuildLimit(cMyID, cUnitTypeSugarFactory)) )  // If I'm not building one and I could be, do it.
      {     // Start a new one
         if ( kbUnitCount(cMyID, cUnitTypeSugarFactory, cUnitStateAlive) < ((kbGetAge()*1) + 1) )
         {  // Less than 1 factory in age 1 (0*2+1), less than 3 in age 2, etc.
            createSimpleBuildPlan(cUnitTypeSugarFactory, 1, 93, true, cEconomyEscrowID, kbBaseGetMainID(cMyID), 1); // Very high pri, just above houses
            aiEcho("Starting a new factory build plan.");
         }
      }
	  //planID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeSoldatSekul);
      //if ( (planID < 0) && (kbUnitCount(cMyID, cUnitTypeSoldatSekul, cUnitStateAlive) < kbGetBuildLimit(cMyID, cUnitTypeSoldatSekul)) )  // If I'm not building one and I could be, do it.
      //{     // Start a new one
       //  if ( kbUnitCount(cMyID, cUnitTypeSoldatSekul, cUnitStateAlive) <  1 )
        // {  // Less than 1 factory in age 1 (0*2+1), less than 3 in age 2, etc.
         //   createSimpleBuildPlan(cUnitTypeSoldatSekul, 1, 93, true, cEconomyEscrowID, kbBaseGetMainID(cMyID), 1); // Very high pri, just above houses
          //  aiEcho("Starting a new school build plan.");
       //  }
     // }	  
      planID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, gMarketUnit);   // Germans market check...only if factories exists
      if ( (planID < 0) && (kbUnitCount(cMyID, gMarketUnit, cUnitStateAlive) < 1) && (kbUnitCount(cMyID, cUnitTypeSugarFactory, cUnitStateAlive) > 0) )
      {
         createSimpleBuildPlan(gMarketUnit, 1, 96, true, cEconomyEscrowID, kbBaseGetMainID(cMyID), 1);  // Just higher than house
         aiEcho("Starting a new market build plan.");
      }
   ...
   // If Russian, at least 1 consulate
	if (kbGetCiv() == cCivRussians)
	{
		planID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeSBarracksJogja);
		if ( (planID < 0) && (kbUnitCount(cMyID, cUnitTypeSBarracksJogja, cUnitStateAlive) < 1) )
		{     // Start a new one
   ...
   else if (  (kbGetCiv() == cCivGermans) || (kbGetCiv() == cCivSpanish) )
	{
		...
		{     // Start a new one
			createSimpleBuildPlan(cUnitTypeSBarracks, 1, 70, false, cMilitaryEscrowID, kbBaseGetMainID(cMyID), 1);
			aiEcho("Starting a new blockhouse build plan.");
		}   
   } 
   ...
   else if ( (kbGetCiv() == cCivGermans) || (kbGetCiv() == cCivOttomans) || (kbGetCiv() == cCivRussians) || (kbGetCiv() == cCivSpanish) )
   {  // Javanese, at least 1 stable
...
minInterval 20
{
   ...
   else if (kbUnitCount(cMyID, cUnitTypeSBarracks, cUnitStateABQ) < 2 && (kbGetAge() > cAge1) )
      buildingToMake = cUnitTypeSBarracks;
   else if (kbUnitCount(cMyID, cUnitTypeSBarracksJogja, cUnitStateABQ) < 2 && (kbGetAge() > cAge1) )
      buildingToMake = cUnitTypeSBarracksJogja;
...
//   aiSetHandler("ageUpHandler", cXSAgeHandler);
   ...
   //xsEnableRule("useSoldatSekulWagon");
...
{  
   int pendapaPlanID = -1;
   int perumahanPlanID = -1;
   int upgradePlanID = -1;


   if (kbUnitCount(cMyID, cUnitTypeMarket, cUnitStateAlive) < 1)
   {  // No gold factories, check for build plan, add one if needed
      perumahanPlanID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeMarket);
      if (perumahanPlanID < 0)
      {
         aiEcho(" ");
         aiEcho("Creating gold factory build plan");
         aiEcho(" ");
         createSimpleBuildPlan(cUnitTypeMarket, 1, 93, true, cEconomyEscrowID, kbBaseGetMainID(cMyID), 1);
      }
      return;
   }
   
   if (kbUnitCount(cMyID, cUnitTypeHouseAztec, cUnitStateAlive) < 4)
   {  // No gold factories, check for build plan, add one if needed
      perumahanPlanID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeHouseAztec);
      if (perumahanPlanID < 0)
      {
         aiEcho(" ");
         aiEcho("Creating gold factory build plan");
         aiEcho(" ");
         createSimpleBuildPlan(cUnitTypeHouseAztec, 1, 83, true, cEconomyEscrowID, kbBaseGetMainID(cMyID), 1);
      }
      return;
   }
   
   if (kbGetAge() < cAge2) // That's all that's needed for the discovery age
      return;
   
   if (kbUnitCount(cMyID, cUnitTypeSPendapaTamtama, cUnitStateAlive) < 1)
   {  // No gold factories, check for build plan, add one if needed
      pendapaPlanID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeSPendapaTamtama);
      if (pendapaPlanID < 0)
      {
         aiEcho(" ");
         aiEcho("Creating gold factory build plan");
         aiEcho(" ");
         createSimpleBuildPlan(cUnitTypeSPendapaTamtama, 1, 93, true, cMilitaryEscrowID, kbBaseGetMainID(cMyID), 1);
      }
      return;
   }
   if (kbUnitCount(cMyID, cUnitTypeSPendapaPrawiran, cUnitStateAlive) < 1)
   {  // Not enough food factories, check for build plan, add some if needed
      pendapaPlanID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeSPendapaPrawiran);
      if (pendapaPlanID < 0)
      {
         aiEcho(" ");
         aiEcho("Creating food factory build plan");
         aiEcho(" ");
         createSimpleBuildPlan(cUnitTypeSPendapaPrawiran, 1, 93, true, cMilitaryEscrowID, kbBaseGetMainID(cMyID), 1);
      }
      return;
   }
   if (kbUnitCount(cMyID, cUnitTypeSPendapaPanyutran, cUnitStateAlive) < 1)
   {  // Not enough wood factories, check for build plan, add some if needed
      pendapaPlanID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeSPendapaPanyutran);
      if (pendapaPlanID < 0)
      {
         aiEcho(" ");
         aiEcho("Creating wood factory build plan");
         aiEcho(" ");
         createSimpleBuildPlan(cUnitTypeSPendapaPanyutran, 1, 93, true, cMilitaryEscrowID, kbBaseGetMainID(cMyID), 1);
      }
      return;
   }
   ...
   if (kbGetAge() < cAge3) // That's all that's needed for the colonial age
      return;

   if (kbUnitCount(cMyID, cUnitTypeSPendapaKanoman, cUnitStateAlive) < 1)
   {  // Not enough gold factories, check for build plan, add some if needed
      pendapaPlanID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeSPendapaKanoman);
      if (pendapaPlanID < 0)
      {
         aiEcho(" ");
         aiEcho("Creating gold factory build plan");
         aiEcho(" ");
         createSimpleBuildPlan(cUnitTypeSPendapaKanoman, 1, 93, true, cMilitaryEscrowID, kbBaseGetMainID(cMyID), 1);
      }
      return;
   }
   if (kbUnitCount(cMyID, cUnitTypeSPendapaDarapaten, cUnitStateAlive) < 1)
   {  // Not enough wood factories, check for build plan, add some if needed
      pendapaPlanID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeSPendapaDarapaten);
      if (pendapaPlanID < 0)
      {
         aiEcho(" ");
         aiEcho("Creating wood factory build plan");
         aiEcho(" ");
         createSimpleBuildPlan(cUnitTypeSPendapaDarapaten, 1, 93, true, cMilitaryEscrowID, kbBaseGetMainID(cMyID), 1);
      }
      return;
   }

   if (kbTechGetStatus(cTechtamtamajajar) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechtamtamajajar);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechtamtamajajar, getUnit(cUnitTypeSPendapaTamtama), cMilitaryEscrowID, 92);
   }
   if (kbTechGetStatus(cTechnyutrajajar) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechnyutrajajar);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechnyutrajajar, getUnit(cUnitTypeSPendapaPanyutran), cMilitaryEscrowID, 92);
   }
   if (kbTechGetStatus(cTechprawiraanom) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechprawiraanom);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechprawiraanom, getUnit(cUnitTypeSPendapaPrawiran), cMilitaryEscrowID, 92);
   }

   if (kbGetAge() < cAge4) // That's all that's needed for the fortress age
      return;

   if (kbTechGetStatus(cTechprawiramadya) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechprawiramadya);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechprawiramadya, getUnit(cUnitTypeSPendapaPrawiran), cMilitaryEscrowID, 92);
   }
   if (kbTechGetStatus(cTechnyutrabekel) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechnyutrabekel);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechnyutrabekel, getUnit(cUnitTypeSPendapaPanyutran), cMilitaryEscrowID, 92);
   }
   if (kbTechGetStatus(cTechtamtamabekel) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechtamtamabekel);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechtamtamabekel, getUnit(cUnitTypeSPendapaTamtama), cMilitaryEscrowID, 92);
   }
   if (kbTechGetStatus(cTechsarageni) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechsarageni);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechsarageni, getUnit(cUnitTypeSPendapaKanoman), cMilitaryEscrowID, 92);
   }
   if (kbTechGetStatus(cTechdarapatibekel) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechdarapatibekel);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechdarapatibekel, getUnit(cUnitTypeSPendapaDarapaten), cMilitaryEscrowID, 92);
   }

   if (kbGetAge() < cAge5) // That's all that's needed for the industrial age
      return;

   if (kbTechGetStatus(cTechprawirasepuh) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechprawirasepuh);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechprawirasepuh, getUnit(cUnitTypeSPendapaPrawiran), cMilitaryEscrowID, 92);
   }
   if (kbTechGetStatus(cTechnyutrapanji) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechnyutrapanji);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechnyutrapanji, getUnit(cUnitTypeSPendapaPanyutran), cMilitaryEscrowID, 92);
   }
   if (kbTechGetStatus(cTechtamtamapanji) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechtamtamapanji);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechtamtamapanji, getUnit(cUnitTypeSPendapaTamtama), cMilitaryEscrowID, 92);
   }
   if (kbTechGetStatus(cTechsarageniageng) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechsarageniageng);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechsarageniageng, getUnit(cUnitTypeSPendapaKanoman), cMilitaryEscrowID, 92);
   }
   if (kbTechGetStatus(cTechdarapatipanji) == cTechStatusObtainable)
   {
      upgradePlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechdarapatipanji);
      if (upgradePlanID < 0)
         createSimpleResearchPlan(cTechdarapatipanji, getUnit(cUnitTypeSPendapaDarapaten), cMilitaryEscrowID, 92);
   }
...
void shipGrantedHandler(int parm=-1) // Event handler
{
   ...
   for (i=0; < totalCards)
   {
      ...
      switch(unitType)
      {
         ...
         //case cUnitTypeSoldatSekulWagon:
         //{
        //    totalValue = 0.0;
         //   if ( (cvOkToBuild == true) && (homeBaseUnderAttack == false) )
         //      totalValue = 2000.0 * qtyAvail;   // Big, but smaller than TC wagon.
...
float getBaseEnemyStrength(int baseID = -1)
{
   ...
   {  // Enemy base, add up military factors normally
      ...
      retVal = retVal + (10.0 * kbBaseGetNumberUnits(owner, baseID, cPlayerRelationEnemy, gFortressUnit));  // 10 points per fort
...
float getPointEnemyStrength(vector loc = cInvalidVector)
{
   ...
   kbUnitQuerySetUnitType(enemyPointQuery, gFortressUnit);
   kbUnitQueryResetResults(enemyPointQuery);
...
float getPointAllyStrength(vector loc = cInvalidVector)
{
   ...
   kbUnitQuerySetUnitType(allyPointQuery, gFortressUnit);
   kbUnitQueryResetResults(allyPointQuery);
...
float getBaseValue(int baseID = -1)
{
   ...
   retVal = retVal + (2000.0 * kbBaseGetNumberUnits(owner, baseID, relation, gFortressUnit));  // 2000 points per fort
